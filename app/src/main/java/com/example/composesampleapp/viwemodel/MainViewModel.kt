...
@Inject constructor(
    ...
    private var _uiState: MutableStateFlow<MainScreenUiState> = MutableStateFlow(MainScreenUiState.Init)
    ...
    suspend fun getWeatherInformation(cityCode: CityCode) {
        ...
        when(val result = repository.getWeatherInformation(cityCode.getCode())){
            is ApiResultOf.Success -> {
                _uiState.emit(MainScreenUiState.Complete(result.value))
            }

            is ApiResultOf.Failure -> {
                _uiState.emit(MainScreenUiState.Error)
            }
        }
    }
}